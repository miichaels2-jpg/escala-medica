...
export default function Trocas() {
  ...
  // Categoria do profissional logado
  ...
  // VAGAS DO MURAL: 100% BLINDADAS E ISOLADAS POR CATEGORIA
  const openShifts = useMemo(() => {
    const todayStr = new Date().toISOString().split('T')[0];

    return shifts.filter(s => {
      // 1. BANIMENTO SUMÁRIO DE CANCELADOS / EXCLUSÕES EM LOTE
      ...
      // 2. Só entra o que for explicitamente VAGA NO MURAL
      const isMuralVaga = s.status === 'vago' || s.is_open === true;
      if (!isMuralVaga || s.professional_id) return false;

      // 3. Descarta datas passadas
      if (s.date && s.date < todayStr) return false;

      // 4. ISOLAMENTO RESTRITO: Se não for gestor geral, SÓ PODE VER VAGA DA SUA PRÓPRIA CATEGORIA
      if (!isManager) {
        const requiredCat = s.target_category || 'medico';
        if (requiredCat !== userCategory) {
          return false; // Bloqueado: médico não vê enfermeiro, enfermeiro não vê médico
        }
      }
  ...
  return (
    <div className="p-4 md:p-8 space-y-6 font-sans bg-slate-950 min-h-screen text-slate-100">
      
      {/* BANNER PRINCIPAL */}
      <div className="rounded-3xl border border-slate-800 bg-gradient-to-r from-slate-950 via-slate-900 to-indigo-950 p-6 text-white shadow-2xl flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
        ...
        <div className="flex items-center gap-3">
          <div className="px-5 py-2.5 rounded-2xl bg-slate-900/80 border border-slate-800 text-center shadow-lg">
            <span className="text-[10px] font-bold uppercase text-slate-400 block">Vagas Disponíveis</span>
      ...
      {/* CARDS DAS VAGAS */}
      {openShifts.length === 0 ? (
        <Card className="p-16 text-center border-slate-800 bg-slate-900 shadow-xl space-y-2">
          <CheckCircle2 className="w-12 h-12 text-emerald-400 mx-auto" />
          <h3 className="font-black text-white text-base">Nenhuma vaga aberta no momento</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {openShifts.map(shift => {
            ...
            return (
              <Card 
                key={shift.id} 
                className="p-5 rounded-3xl border-2 border-amber-500/40 bg-slate-900/90 flex flex-col justify-between space-y-4 shadow-xl hover:border-amber-400 transition-all"
              >
                <div className="space-y-3">
                  <div className="flex items-start justify-between gap-2 border-b border-slate-800 pb-3">
                    <div>
                      ...
                      <h3 className="font-black text-base text-white mt-2">
                        {sector?.name || 'Setor Hospitalar'}
                      </h3>
                    </div>

                    <span className="text-xs font-mono font-black px-2.5 py-1 rounded-xl bg-slate-950 text-sky-400 border border-slate-800">
                  ...
                  <div className="space-y-1.5 text-xs text-slate-300">
                    ...
                    {shift.notes && (
                      <p className="text-[11px] text-slate-400 italic mt-2 bg-slate-950 p-2.5 rounded-xl border border-slate-800">
                        "{shift.notes}"
                      </p>
                    )}
                  </div>
                ...
                <div className="pt-3 border-t border-slate-800 flex items-center gap-2">
                  <Button
                    ...
                    disabled={submitting}
                    className="flex-1 h-10 bg-amber-600 hover:bg-amber-500 text-white font-black text-xs rounded-xl shadow-lg gap-2"
                  >
                  ...
                  {isManager && (
                    <Button
                      ...
                      title="Excluir vaga"
                      className="h-10 w-10 p-0 text-slate-500 hover:text-rose-400 hover:bg-rose-500/10 rounded-xl"
                    >
