...
export default function Configuracoes() {
  ...
  const handleAutoFillFromContract = () => {
    setNewUnitForm(prev => ({
      ...prev,
      name: contractForm.name || prev.name,
      cnpj: contractForm.cnpj || prev.cnpj
    }));
  };
                ...
                <form onSubmit={handleSaveNewUnit} className="space-y-4 py-2 text-xs">
                  ...
                  {!editingUnit && contractForm.name && (
                    <Button 
                      ...
                      className="w-full bg-sky-50 hover:bg-sky-100 dark:bg-sky-950/30 dark:hover:bg-sky-900/50 text-sky-700 dark:text-sky-400 font-bold border border-sky-200 dark:border-sky-800 cursor-pointer h-9 mb-2"
                    >
