...
import { Calendar, Repeat, DollarSign, User, Clock, MapPin, CheckCircle2, ArrowLeft } from 'lucide-react';
...
export default function MinhaEscala() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const profs = await base44.entities.Professional.filter(f, '-created_date', 200);
        const me = profs.find((p) => p.user_id === user?.id || (p.email && p.email === user?.email));
        setMyProfessional(me || null);
        setProfessionals(profs);
        const profId = me?.id;
        const shifts = await base44.entities.Shift.filter(f, 'date', 500);
        setCompanyShifts(shifts);
        const mine = shifts
          .filter((s) => s.professional_id === profId || (!profId && s.professional_name === (user?.full_name || '')))
          .filter((s) => s.status === 'confirmado' || s.status === 'pendente')
          .sort((a, b) => (a.date || '').localeCompare(b.date || ''));
        setMyShifts(mine);
      } catch (e) {}
    })();
  }, [loading, companyId, user]);

  if (loading) return <div className="p-8 text-slate-400">Carregando...</div>;

  const now = new Date();
  const upcoming = myShifts.filter((s) => (getShiftInterval(s)?.end || new Date(0)) >= new Date(now.getFullYear(), now.getMonth(), now.getDate()));
  const next = upcoming[0];

  const availableOpenShifts = myProfessional ? companyShifts.filter((shift) => {
    ...
    const matchesRole = !specialty || specialty.includes(targetLower) || targetLower.includes(specialty) || category.includes(targetLower) || targetLower.includes(category);
  ...
  const totalHours = myShifts.reduce((s, sh) => s + (sh.duration_hours || 12), 0);
  ...
  const handleAcceptOpenShift = async (shiftId) => {
    try {
      ...
      const updated = await base44.entities.Shift.filter(companyId ? { company_id: companyId, ...(unitId ? { unit_id: unitId } : {}) } : {}, 'date', 500);
      setCompanyShifts(updated);
      const mine = updated
        .filter((s) => s.professional_id === myProfessional.id || (!myProfessional && s.professional_name === (user?.full_name || '')))
        .filter((s) => s.status === 'confirmado' || s.status === 'pendente')
        .sort((a, b) => (a.date || '').localeCompare(b.date || ''));
      setMyShifts(mine);
    } catch (e) {
  ...
  const handleConfirmShift = async (shiftId) => {
    try {
      ...
      const updated = await base44.entities.Shift.filter(companyId ? { company_id: companyId, ...(unitId ? { unit_id: unitId } : {}) } : {}, 'date', 500);
      const mine = updated.filter((s) => s.professional_id === myProfessional?.id || (!myProfessional && s.professional_name === (user?.full_name || ''))).filter((s) => s.status === 'confirmado' || s.status === 'pendente').sort((a, b) => (a.date || '').localeCompare(b.date || ''));
      setMyShifts(mine);
    } catch (e) {
  ...
  const handleCheckIn = async (shift) => {
    ...
    navigator.geolocation.getCurrentPosition(async (position) => {
      try {
        const allowed = position?.coords && typeof position.coords.latitude === 'number';
        if (!allowed) {
          alert('Não foi possível confirmar a localização da unidade.');
          return;
        }
      ...
      <div className="w-full max-w-5xl bg-slate-50 min-h-full flex flex-col">
        ...
        <div className="bg-white px-5 py-4 flex items-center justify-between border-b border-slate-100 sticky top-0 z-10">
          <div>
            ...
            <div className="text-base font-bold text-slate-800">{myProfessional?.name || user?.full_name || user?.email}</div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <div className="w-10 h-10 rounded-full bg-sky-100 flex items-center justify-center text-sm font-semibold text-sky-700">
              {(myProfessional?.name || user?.full_name || '?').slice(0, 2).toUpperCase()}
            </div>
          </div>
        </div>
        ...
        <div className="mx-auto flex-1 w-full max-w-4xl p-4 space-y-4 overflow-y-auto pb-24 md:p-8">
          <div className="flex items-center justify-between gap-2 rounded-2xl border border-sky-100 bg-sky-50 px-3 py-2">
            <div>
              ...
              <div className="text-sm font-semibold text-slate-700">{myProfessional?.name || user?.full_name || 'Profissional'}</div>
            </div>
            {!isManager && (
              <span className="rounded-full bg-white px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.16em] text-sky-700">Perfil profissional</span>
            )}
          </div>
          ...
          {next ? (
            <div className="bg-sky-600 text-white p-5 rounded-2xl shadow-sm">
              <div className="text-xs opacity-80 flex items-center gap-1.5"><Clock className="w-3.5 h-3.5" /> Próximo Plantão</div>
              ...
              <div className="text-xs mt-2 opacity-90 flex items-center gap-1.5"><MapPin className="w-3.5 h-3.5" /> {company?.name || 'Hospital'} · {next.sector_name || '—'}</div>
            </div>
          ) : (
                  ...
                  {availableOpenShifts.map((shift) => (
                    <div key={shift.id} className="bg-amber-50 border border-amber-200 rounded-2xl p-4 space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="rounded-full bg-white px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.16em] text-amber-700">{shift.shift_type}</span>
                      </div>
                      <div className="text-xs text-slate-600">{shift.start_time} às {shift.end_time}</div>
                      <Button size="sm" className="w-full text-xs h-8" onClick={() => handleAcceptOpenShift(shift.id)}>Aceitar vaga</Button>
                    </div>
                  ))}
                </div>
              )}
              ...
              {upcoming.map((s) => (
                <div key={s.id} className="bg-white border border-slate-200 rounded-2xl p-4 space-y-2.5 shadow-sm">
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded font-medium ${shiftTypeStyle[s.shift_type] || 'bg-slate-100 text-slate-600'}`}>{s.duration_hours || 12}h</span>
                  </div>
                  <div className="text-sm font-semibold text-slate-800">{s.start_time} às {s.end_time} · {s.sector_name || '—'}</div>
                  <div className="text-xs text-slate-500 flex items-center gap-1.5"><MapPin className="w-3.5 h-3.5" /> {company?.name || 'Hospital'}</div>
                  <div className="flex items-center gap-1.5 text-xs">
                    {s.status === 'confirmado' || s.checked_in ? (
                      <span className="text-emerald-600 flex items-center gap-1 font-medium"><CheckCircle2 className="w-3.5 h-3.5" /> {s.checked_in ? 'Check-in validado' : 'Confirmado'}</span>
                    ) : (
                      <span className="text-amber-600 flex items-center gap-1 font-medium"><Clock className="w-3.5 h-3.5" /> Aguardando confirmação</span>
                    )}
                  </div>
                  <div className="flex gap-2 pt-1">
                    <Button variant="outline" size="sm" className="flex-1 text-xs h-8" onClick={() => setSwapShift(s)}>Solicitar Troca</Button>
                    {s.status !== 'confirmado' ? (
                      <Button size="sm" className="flex-1 text-xs h-8" onClick={() => handleConfirmShift(s.id)}>Confirmar</Button>
                    ) : (
                      <Button size="sm" className="flex-1 text-xs h-8" onClick={() => handleCheckIn(s)}>{s.checked_in ? 'Confirmado' : 'Check-in'}</Button>
                    )}
                  </div>
                </div>
              ))}
          ...
          {tab === 'extrato' && (
            <div className="bg-white p-5 rounded-2xl border border-slate-200 space-y-3">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-lg font-bold text-emerald-600">{myShifts.filter((s) => s.status === 'confirmado').length}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-slate-500">Pendentes</span>
                <span className="text-lg font-bold text-amber-600">{myShifts.filter((s) => s.status === 'pendente').length}</span>
              </div>
          ...
          {tab === 'perfil' && (
            <div className="bg-white p-5 rounded-2xl border border-slate-200 space-y-3">
              <div className="text-center">
                ...
                <div className="font-semibold text-slate-800 mt-2">{myProfessional?.name || user?.full_name}</div>
                <div className="text-xs text-slate-500">{user?.email}</div>
              </div>
              {myProfessional && (
                <div className="space-y-1.5 pt-2 border-t border-slate-100 text-sm">
                  {myProfessional.specialty && <div className="text-slate-600">Especialidade: <span className="font-medium">{myProfessional.specialty}</span></div>}
                  {myProfessional.document && <div className="text-slate-600">CBO: <span className="font-medium">{myProfessional.document}{myProfessional.document_uf ? ` / ${myProfessional.document_uf}` : ''}</span></div>}
                  {myProfessional.phone && <div className="text-slate-600">Telefone: <span className="font-medium">{myProfessional.phone}</span></div>}
                </div>
              )}
              <Link to="/configuracoes"><Button variant="outline" size="sm" className="w-full">Editar perfil</Button></Link>
            </div>
          )}
        </div>
          ...
          {[
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center text-[10px] gap-0.5 ${tab === item.key ? 'text-sky-600' : 'text-slate-400'}`}
              >
