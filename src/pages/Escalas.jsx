...
import { 
  ...
  Clock, Building2, AlertTriangle, CheckCircle2, 
  Trash2, X, Minimize2, Sparkles, CheckCheck, Send, 
  MousePointerClick, HeartPulse, UserPlus, SlidersHorizontal,
  Flame, Radio, ArrowRight, MonitorPlay, GripVertical, 
  Printer, Sun, Moon, Columns3
} from 'lucide-react';
...
// Lógica Implacável de Fuso e Horário
...
function getShiftDates(shift) {
  ...
  if (end <= start) end.setDate(end.getDate() + 1); // Vira a noite
  return { start, end };
}

function getShiftStatus(shift, now = new Date()) {
  const isVago = shift.status === 'vago' || !shift.professional_id;
  const { start, end } = getShiftDates(shift);
  
  if (isVago) {
     if (now >= end) return { code: 'perdida', label: 'VAGA PERDIDA', dot: 'bg-slate-400', text: 'text-slate-500' };
     return { code: 'vaga', label: 'VAGA ABERTA', dot: 'bg-rose-500 animate-pulse', text: 'text-rose-600 dark:text-rose-400' };
  }
  if (now >= end) return { code: 'concluido', label: 'CONCLUÍDO', dot: 'bg-slate-400', text: 'text-slate-500' };
  if (now >= start && now < end) return { code: 'aovivo', label: 'AO VIVO AGORA', dot: 'bg-emerald-500 animate-ping', text: 'text-emerald-600 dark:text-emerald-400' };
  return { code: 'programado', label: 'PROGRAMADO', dot: 'bg-sky-500', text: 'text-sky-600 dark:text-sky-400' };
}
...
export default function Escalas() {
  ...
  // CÁLCULO DE TV CCO À PROVA DE FALHAS
  const tvShiftsDetailed = useMemo(() => {
    ...
    shifts.forEach(shift => {
      ...
      } else if (minsAgo >= 0 && minsAgo <= 60) {
        concluidosRecentes.push({ shift, minutesAgo: Math.round(minsAgo) });
  ...
  // CÁLCULO PLANTÃO DO DIA (Tudo que toca o dia de hoje)
  const todayShiftsForTable = useMemo(() => {
    const todayStart = new Date(); todayStart.setHours(0,0,0,0);
    const todayEnd = new Date(); todayEnd.setHours(23,59,59,999);

    return shifts.filter(shift => {
      if (selectedSectorId !== 'todos' && String(shift.sector_id) !== String(selectedSectorId)) return false;
      const { start, end } = getShiftDates(shift);
      return start <= todayEnd && end >= todayStart;
    }).sort((a, b) => a.start_time.localeCompare(b.start_time));
  }, [shifts, selectedSectorId, liveNow]);
    ...
    <div className={`p-3 md:p-6 space-y-4 font-sans bg-slate-100 dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors duration-200 ${activeTab === 'tv' ? 'fixed inset-0 z-50 bg-slate-950 text-white overflow-y-auto p-6 md:p-8' : ''}`}>
      ...
      {activeTab === 'mensal' && (
        <div className="flex flex-col lg:flex-row gap-4 items-start">
          ...
          {/* GRADE MENSAL COM SUB-COLUNAS DE TURNOS E CORES EXPLÍCITAS */}
            ...
            <div className="grid grid-cols-7 divide-x divide-y divide-slate-200 dark:divide-slate-800">
              ...
              {daysInMonth.map(dateObj => {
                ...
                const renderCard = (shift) => {
                  const prof = shift.professional_id ? professionalMap[String(shift.professional_id)] : null;
                  const status = getShiftStatus(shift, liveNow);
                  const realSpec = extractSpecialty(shift, prof);

                  let cardClass = 'bg-white dark:bg-slate-950 border-slate-200 dark:border-slate-800 hover:border-slate-400';
                  if (status.code === 'perdida' || status.code === 'concluido') cardClass = 'bg-slate-100 dark:bg-slate-900/60 border-slate-200 dark:border-slate-800 opacity-60 grayscale hover:grayscale-0';
                  else if (status.code === 'vaga') cardClass = 'bg-rose-50 dark:bg-rose-950/30 border-rose-300 dark:border-rose-800/60';
                  else if (status.code === 'aovivo') cardClass = 'bg-emerald-50 dark:bg-emerald-950/20 border-emerald-400 dark:border-emerald-600 ring-1 ring-emerald-500/50';

                  return (
                    <div key={shift.id} onClick={(e) => { e.stopPropagation(); if (isManager) { setEditingShiftId(shift.id); setFormData({ date: shift.date || '', sector_id: shift.sector_id || '', target_specialty: realSpec, start_time: shift.start_time || '07:00', end_time: shift.end_time || '19:00', shift_type: shift.shift_type || 'diurno', action_type: (shift.status === 'vago' || !prof) ? 'mural' : 'alocar', professional_id: shift.professional_id || '', notes: shift.notes || '' }); setModalOpen(true); } }} className={`p-1.5 rounded-xl border text-[10px] shadow-sm cursor-pointer transition-all ${cardClass}`}>
                      ...
                      <div className="font-black truncate leading-tight text-slate-900 dark:text-white">
                        {status.code.includes('vaga') || status.code === 'perdida' ? `⚠️ ${status.label}` : formatFullName(prof?.name)}
                      </div>
                      ...
                      return (
                        <tr key={shift.id} className="hover:bg-slate-50 dark:hover:bg-slate-850/60 transition-colors">
                          ...
                          <td className="py-3 px-4 font-black text-slate-900 dark:text-slate-100">
                            {status.code.includes('vaga') || status.code === 'perdida' ? <span className="text-rose-600 dark:text-rose-400">⚠️ {status.label}</span> : formatFullName(prof?.name)}
                          </td>
                          <td className="py-3 px-4 text-slate-700 dark:text-slate-300 font-semibold">{realSpecialty}</td>
                          <td className="py-3 px-4 text-center">
                            <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black tracking-wider ${status.dot.replace('bg-', 'bg-opacity-20 text-').replace('animate-ping', '').replace('animate-pulse', '')}`}>
                              <span className={`w-2 h-2 rounded-full ${status.dot}`}></span> {status.label}
                            </span>
                          </td>
                        </tr>
                      );
          ...
          <div className="flex-1 overflow-y-auto p-8 bg-gray-300 print:p-0 print:bg-white flex justify-center">
            ...
            <div id="print-section" className="w-full max-w-[297mm] bg-white text-black p-8 shadow-2xl print:shadow-none print:p-0 print:max-w-full font-sans border border-gray-300">
              ...
              <table className="w-full border-collapse border-2 border-black text-xs mb-8">
                <thead className="bg-gray-100 text-black uppercase text-[10px] font-black border-b-2 border-black">
                  <tr>
