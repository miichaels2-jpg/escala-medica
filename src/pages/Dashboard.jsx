      ...
      <div className="bg-gradient-to-r from-slate-950 via-slate-900 to-sky-950 text-white p-6 md:p-8 rounded-3xl border border-slate-800 shadow-2xl flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-1.5">
          ...
          <p className="text-xs md:text-sm text-slate-300 font-medium max-w-2xl">
            Monitoramento em tempo real de escalas, cobertura médica hospitalar e custo operacional diário da unidade selecionada.
          </p>
        </div>
          ...
          <div>
            ...
            <div className="space-y-2 text-xs">
              ...
              <div className="flex justify-between items-center p-2.5 rounded-xl bg-slate-50 dark:bg-slate-850/50 font-medium">
                ...
                <strong className={vacantShifts.length > 0 ? "text-rose-600 font-mono font-black" : "text-slate-400 font-mono"}>{vacantShifts.length} postos</strong>
              </div>
            </div>
          </div>
      ...
      <Card className="p-6 rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm space-y-4">
        <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
          ...
          <span className="text-xs font-black px-3 py-1 rounded-full bg-sky-50 text-sky-700 dark:bg-sky-950 dark:text-sky-300">
            {todayShifts.length} turno(s) na unidade
          </span>
        </div>

        {todayShifts.length === 0 ? (
          <div className="py-12 text-center text-sm text-slate-400">
            Nenhum plantão ativo ou agendado para a data de hoje nesta unidade.
          </div>
        ) : (
            ...
            {todayShifts.map((shift) => {
              ...
              return (
                <div
                  key={shift.id}
                  className={`p-4 rounded-2xl border transition-all ${
                    isVago ? 'border-rose-400 bg-rose-50/60 dark:bg-rose-950/20 shadow-sm' : isActive ? 'border-emerald-400 bg-emerald-50/60 dark:bg-emerald-950/20 shadow-sm ring-1 ring-emerald-400/40' : isFinished ? 'border-slate-200 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-850/40 opacity-70' : 'border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900'
                  }`}
                >
                  <div className="flex items-center justify-between gap-2 mb-2">
                    ...
                    <span className={`text-[9px] font-black px-2.5 py-0.5 rounded-full uppercase ${isVago ? 'bg-rose-600 text-white animate-pulse' : isActive ? 'bg-emerald-600 text-white animate-pulse' : isFinished ? 'bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-300' : 'bg-sky-100 text-sky-800 dark:bg-sky-950 dark:text-sky-300'}`}>
