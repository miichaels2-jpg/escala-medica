...
import { 
  ...
  Radio, TrendingUp, Flame, ChevronRight, X
} from 'lucide-react';
...
function computeShiftLiveStatus(shift, liveNowDate) {
  ...
  if (nowMs >= startMs && nowMs < endMs) {
    ...
    return { state: 'active', isLive: true, isConcluded: false, isProgrammed: false, remainingMinutes: diffMin, detail: `Resta ${Math.floor(diffMin / 60)}h ${diffMin % 60}m` };
  }

  if (nowMs >= endMs) return { state: 'concluded', isLive: false, isConcluded: true, isProgrammed: false, detail: `Concluído às ${shift.end_time}` };
  return { state: 'upcoming', isLive: false, isConcluded: false, isProgrammed: true, detail: `Inicia às ${shift.start_time}` };
}
...
export default function Painel() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      // ========================================================
      // ISOLAMENTO BLINDADO DE DADOS (FRONTEND)
      // Garante que a Unidade Dois NÃO exiba NADA da Unidade Um
      // ========================================================
      ...
      // Profissionais continuam globais pois atendem vários hospitais do mesmo contrato
      setProfessionals(p || []);
    } catch (e) {
  ...
  const { profById, profByName } = useMemo(() => {
    ...
    (professionals || []).forEach((p) => {
      ...
      const mergedProf = { ...p, monthly_salary: meta.monthly_salary !== undefined ? meta.monthly_salary : p.monthly_salary, hourly_rate: meta.hourly_rate !== undefined ? meta.hourly_rate : p.hourly_rate, daily_rate: meta.daily_rate !== undefined ? meta.daily_rate : p.daily_rate, remuneration_type: meta.remuneration_type || p.remuneration_type || 'mensal' };
  ...
  const todayShifts = useMemo(() => {
    return (shifts || []).filter((s) => {
      ...
      return { ...s, lifecycle: { state: liveStatus.state, detail: liveStatus.detail } };
    }).sort((a, b) => {
  ...
  const vacantShifts = useMemo(() => {
    ...
    }).map(s => ({ ...s, sectorName: toTitleCase(s.sector_name || sectors.find(sec => String(sec.id) === String(s.sector_id))?.name || 'Setor Geral'), formattedDate: fmtDate(s.date) }));
  ...
  const todayFinancials = useMemo(() => {
    ...
    todayShifts.forEach((s) => {
      ...
      let shiftCost = 0;
      if (prof) {
        const remType = prof.remuneration_type || 'mensal'; const salary = safeNumber(prof.monthly_salary, 1672); const daily = safeNumber(prof.daily_rate, 0); const hourly = safeNumber(prof.hourly_rate, 0);
        if (remType === 'hora') { const hours = Number(s.duration_hours) || 12; shiftCost = hours * (hourly > 0 ? hourly : (salary / 220)); } 
        else if (remType === 'diaria') { shiftCost = daily > 0 ? daily : (salary / 20); } 
        else { shiftCost = salary / 20; }
      } else { if (s.professional_name && !s.professional_name.toLowerCase().includes('vaga')) { shiftCost = 1672 / 20; } }
      plannedValue += shiftCost;
      if (s.lifecycle.state === 'active' || s.lifecycle.state === 'concluded') executedValue += shiftCost;
    });
  ...
  const sectorsCoverage = useMemo(() => {
    const map = {};
    (sectors || []).forEach((sec) => { map[sec.id] = { id: sec.id, name: toTitleCase(sec.name), specialty: sec.specialty || 'Geral', total: 0, active: 0, upcoming: 0, vacant: 0 }; });
    todayShifts.forEach((s) => {
  ...
  const hourlyCurveData = useMemo(() => {
    const buckets = [ { label: '06h', count: 0 }, { label: '08h', count: 0 }, { label: '10h', count: 0 }, { label: '12h', count: 0 }, { label: '14h', count: 0 }, { label: '16h', count: 0 }, { label: '18h', count: 0 }, { label: '20h', count: 0 }, { label: '22h', count: 0 }, { label: '00h', count: 0 }, { label: '02h', count: 0 }, { label: '04h', count: 0 } ];
    todayShifts.forEach(s => { const h = parseInt((s.start_time || '07:00').split(':')[0], 10); const idx = Math.min(11, Math.max(0, Math.floor((h >= 6 ? h - 6 : h + 18) / 2))); buckets[idx].count += 1; });
    const max = Math.max(1, ...buckets.map(b => b.count));
    return buckets.map(b => ({ ...b, height: Math.round((b.count / max) * 100) }));
  }, [todayShifts]);

  const handleResolveAlert = (vs) => {
    if (vs?.sector_id) window.localStorage.setItem('scale_filter_sector_id', String(vs.sector_id));
    if (vs?.id) window.localStorage.setItem('scale_auto_open_shift_id', String(vs.id));
    navigate('/escalas');
  };

  const handleNavigateToSectorScale = (secId) => {
    if (secId) window.localStorage.setItem('scale_filter_sector_id', String(secId));
    navigate('/escalas');
  };
        ...
        <div className="flex items-center justify-between border-b border-slate-800 pb-4 shrink-0 pr-12 lg:pr-0">
          ...
          <div className="hidden lg:flex items-center gap-5">
            <div className={`px-4 py-2.5 rounded-2xl border flex items-center gap-3 ${vacantShifts.length > 0 ? 'bg-rose-500/10 border-rose-500/40 text-rose-300' : 'bg-emerald-500/10 border-emerald-500/40 text-emerald-300'}`}>
              ...
              <div><div className="text-xs font-black uppercase tracking-wider">{vacantShifts.length > 0 ? 'Alerta Assistencial' : 'Operação Estável'}</div><div className="text-[10px] font-bold opacity-80">{vacantShifts.length > 0 ? `${vacantShifts.length} vaga(s) desocupada(s)` : 'Todos os postos cobertos'}</div></div>
            </div>
  ...
  return (
    <div className="p-4 md:p-8 space-y-6 font-sans bg-slate-900/40 min-h-screen text-slate-900 dark:text-slate-100">
      ...
      <div className="bg-gradient-to-r from-slate-950 via-slate-900 to-sky-950 text-white p-6 md:p-8 rounded-3xl border border-slate-800 shadow-2xl flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <Button onClick={openTvMode} className="bg-sky-600 hover:bg-sky-500 text-white font-black text-xs h-12 px-6 rounded-2xl shadow-xl gap-2.5 transition-all hover:scale-105 shrink-0 cursor-pointer">
      ...
      {unsubmittedSectors.length > 0 && isManager && (
        <div className="p-4 rounded-3xl bg-amber-500/10 border border-amber-500/30 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4 animate-in fade-in">
          <div className="flex items-start sm:items-center gap-3">
            <div className="p-2.5 rounded-2xl bg-amber-500 text-white shrink-0 shadow-sm"><AlertTriangle className="w-5 h-5" /></div>
            <div>
              ...
              <div className="flex flex-wrap items-center gap-1.5 mt-1">
                ...
                {unsubmittedSectors.map(s => (
                  <button key={s.id} onClick={() => handleNavigateToSectorScale(s.id)} className="text-[10px] font-black uppercase px-2 py-0.5 rounded-lg bg-amber-500/20 text-amber-800 dark:text-amber-200 hover:bg-amber-500/30 transition-all cursor-pointer border border-amber-500/30">{s.name}</button>
                ))}
              </div>
            </div>
          </div>
          <Button onClick={() => navigate('/escalas')} className="h-9 bg-amber-600 hover:bg-amber-500 text-white font-black text-xs px-4 rounded-xl shadow-md shrink-0 cursor-pointer gap-1.5"><Send className="w-3.5 h-3.5" /> Abrir Escalas</Button>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="p-5 rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm">
          ...
          <div className="text-3xl font-black text-slate-900 dark:text-white mt-3">{todayShifts.length} <span className="text-sm font-bold text-slate-500">turnos</span></div>
          <p className="text-[11px] text-slate-500 mt-1 font-medium">{upcomingList.length} a iniciar · {concludedList.length} concluídos</p>
        </Card>
        <Card className="p-5 rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm">
          <div className="flex items-center justify-between"><span className="text-xs font-black uppercase tracking-wider text-slate-500">Corpo Clínico Credenciado</span><span className="p-2 rounded-xl bg-indigo-50 dark:bg-indigo-950 text-indigo-600 dark:text-indigo-400"><Users className="w-4 h-4" /></span></div>
          <div className="text-3xl font-black text-slate-900 dark:text-white mt-3">{professionals.length} <span className="text-sm font-bold text-slate-500">profissionais</span></div>
          <p className="text-[11px] text-slate-500 mt-1 font-medium">Equipe vinculada às escalas</p>
        </Card>
        <Card className="p-5 rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="p-6 rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm lg:col-span-2 space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {sectorsCoverage.map((sec) => {
              const hasShifts = sec.total > 0; const percent = hasShifts ? Math.round((sec.active / sec.total) * 100) : 0; const hasVacant = sec.vacant > 0;
              return (
                <div key={sec.id} className={`p-4 rounded-2xl border transition-all ${hasVacant ? 'border-rose-300 bg-rose-50/50 dark:border-rose-900/60 dark:bg-rose-950/20 shadow-sm' : sec.active > 0 ? 'border-emerald-200 bg-emerald-50/30 dark:border-emerald-900/50 dark:bg-emerald-950/20' : 'border-slate-200 dark:border-slate-800/80 bg-slate-50/50 dark:bg-slate-900/40'}`}>
                  <div className="flex items-center justify-between">
                    <strong className="text-sm font-black text-slate-900 dark:text-white truncate">{sec.name}</strong>
                    <span className={`text-[10px] font-black px-2.5 py-0.5 rounded-full ${hasVacant ? 'bg-rose-100 text-rose-800 dark:bg-rose-950 dark:text-rose-300' : sec.active > 0 ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300' : 'bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-400'}`}>
                      {hasShifts ? `${sec.active} ativo(s)` : 'Sem escala hoje'}
                    </span>
                  </div>
                  <div className="flex items-center justify-between text-xs text-slate-500 mt-2.5 font-medium"><span>Ocupação do posto</span><span className="font-mono font-black text-slate-900 dark:text-white">{hasShifts ? `${sec.active} de ${sec.total} turnos (${percent}%)` : '0 / 0'}</span></div>
                  <div className="w-full h-2 bg-slate-200 dark:bg-slate-800 rounded-full mt-2 overflow-hidden"><div className={`h-full transition-all duration-500 ${hasVacant ? 'bg-rose-500' : sec.active > 0 ? 'bg-emerald-500' : 'bg-transparent'}`} style={{ width: `${hasShifts ? Math.min(100, Math.max(5, percent)) : 0}%` }} /></div>
                  {hasVacant ? <div className="text-[11px] text-rose-600 dark:text-rose-400 font-black mt-2 flex items-center gap-1 animate-pulse"><AlertTriangle className="w-3.5 h-3.5" /> {sec.vacant} vaga(s) com desfalque médico</div> : hasShifts ? <div className="text-[10px] text-slate-400 mt-2 font-medium">{sec.upcoming} plantonista(s) a assumir a seguir</div> : <div className="text-[10px] text-slate-400 mt-2 italic">Nenhum plantão agendado para este setor hoje</div>}
                </div>
              );
            })}
          </div>
        ...
        <Card className="p-6 rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm flex flex-col justify-between space-y-4">
          <div>
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
              <h3 className="font-black text-base text-slate-900 dark:text-white flex items-center gap-2"><TrendingUp className="w-5 h-5 text-emerald-600" /> Eficiência de Cobertura</h3>
            ...
            <div className="py-6 flex flex-col items-center justify-center relative">
              <svg className="w-44 h-44 transform -rotate-90" viewBox="0 0 120 120">
                ...
                <circle cx="60" cy="60" r="48" className={globalFillRate === 100 ? "text-emerald-500" : "text-sky-500"} strokeWidth="12" strokeDasharray={2 * Math.PI * 48} strokeDashoffset={2 * Math.PI * 48 * (1 - globalFillRate / 100)} strokeLinecap="round" stroke="currentColor" fill="transparent" />
              </svg>
              <div className="absolute flex flex-col items-center justify-center text-center">
                <span className="text-4xl font-black font-mono tracking-tight text-slate-900 dark:text-white">{globalFillRate}%</span><span className="text-[10px] font-black uppercase text-slate-400 tracking-wider mt-0.5">Coberto</span>
              </div>
            </div>
            <div className="space-y-2 text-xs">
              <div className="flex justify-between items-center p-2.5 rounded-xl bg-slate-50 dark:bg-slate-850/50 font-medium"><span className="text-slate-500">Postos com Médico Ativo:</span><strong className="text-emerald-600 font-mono font-black">{activeNowList.length} turnos</strong></div>
              <div className="flex justify-between items-center p-2.5 rounded-xl bg-slate-50 dark:bg-slate-850/50 font-medium"><span className="text-slate-500">Vagas Descobertas:</span><strong className={vacantShifts.length > 0 ? "text-rose-600 font-mono font-black" : "text-slate-400 font-mono"}>{vacantShifts.length} postos</strong></div>
            </div>
          </div>
          <div className="pt-3 border-t border-slate-100 dark:border-slate-800">
            <span className="text-[10px] font-black uppercase tracking-wider text-slate-400 block mb-2">Distribuição da Carga Horária 24h</span>
            <div className="flex items-end justify-between h-14 gap-1">
              {hourlyCurveData.map((b, i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-1">
                  <div className="w-full rounded-t-sm bg-gradient-to-t from-sky-600 to-indigo-500" style={{ height: `${Math.max(12, b.height)}%` }} title={`${b.label}: ${b.count} plantões`} />
      ...
      <Card className="p-6 rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm space-y-4">
        <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
          ...
          <span className="text-xs font-black px-3 py-1 rounded-full bg-sky-50 text-sky-700 dark:bg-sky-950 dark:text-sky-300">{todayShifts.length} turno(s) na unidade</span>
        </div>

        {todayShifts.length === 0 ? (
          <div className="py-12 text-center text-sm text-slate-400">Nenhum plantão ativo ou agendado para a data de hoje nesta unidade.</div>
        ) : (
          <div className="grid gap-3.5 sm:grid-cols-2 lg:grid-cols-3">
            {todayShifts.map((shift) => {
              const state = shift.lifecycle.state; const isActive = state === 'active'; const isFinished = state === 'concluded'; const isVago = !shift.professional_id || shift.status === 'vago' || (shift.professional_name || '').toLowerCase().includes('vaga');
              return (
                <div key={shift.id} className={`p-4 rounded-2xl border transition-all ${isVago ? 'border-rose-400 bg-rose-50/60 dark:bg-rose-950/20 shadow-sm' : isActive ? 'border-emerald-400 bg-emerald-50/60 dark:bg-emerald-950/20 shadow-sm ring-1 ring-emerald-400/40' : isFinished ? 'border-slate-200 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-850/40 opacity-70' : 'border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900'}`}>
                  <div className="flex items-center justify-between gap-2 mb-2">
                    <span className="text-xs font-bold font-mono text-slate-600 dark:text-slate-300">{shift.start_time} às {shift.end_time}</span>
                    <span className={`text-[9px] font-black px-2.5 py-0.5 rounded-full uppercase ${isVago ? 'bg-rose-600 text-white animate-pulse' : isActive ? 'bg-emerald-600 text-white animate-pulse' : isFinished ? 'bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-300' : 'bg-sky-100 text-sky-800 dark:bg-sky-950 dark:text-sky-300'}`}>{isVago ? '⚠️ Vaga Aberta' : isActive ? '● Ativo no Plantão' : isFinished ? '✓ Concluído' : '⏳ Programado'}</span>
                  </div>
                  <div className="font-black text-sm text-slate-900 dark:text-white truncate">{isVago ? <span className="text-rose-600">Vaga em Aberto</span> : toTitleCase(shift.professional_name)}</div>
                  <div className="flex items-center justify-between text-xs text-slate-500 mt-1"><span className="truncate">{toTitleCase(shift.sector_name) || 'Setor Geral'}</span><span className="text-[10px] font-semibold text-slate-400">{shift.lifecycle.detail}</span></div>
                </div>
              );
            })}
          </div>
