...
export default function CentralInteligenciaHospitalar() {
  ...
  /* ============================================================
     FINANCEIRO (Corrigido cruzamento de dados com faturamento)
     ============================================================ */
  ...
  const financialData = useMemo(() => {
    ...
    filteredShifts.forEach((shift) => {
      ...
      const professional = professionalId ? professionalMap[professionalId] : null;

      const remunerationType = getRemunerationType(professional);

      let rate = null;

      if (remunerationType === REMUNERATION_TYPE.HOUR) {
        rate = professional?.hourly_rate ?? professional?.hourlyRate ?? shift?.hourly_rate ?? 120;
      } else if (remunerationType === REMUNERATION_TYPE.DAILY) {
        rate = professional?.daily_rate ?? professional?.dailyRate ?? 1500;
      } else if (remunerationType === REMUNERATION_TYPE.MONTHLY) {
        const monthly = Number(professional?.monthly_salary ?? professional?.monthlySalary ?? 18000);
        rate = monthly / 30 / 12; // Proporcional base
      }

      const numericRate = rate === null || rate === '' ? null : safeNumber(rate);

      if (numericRate !== null && numericRate >= 0) {
        const cost = remunerationType === REMUNERATION_TYPE.MONTHLY
          ? numericRate * hours
          : remunerationType === REMUNERATION_TYPE.DAILY
          ? numericRate
          : hours * numericRate;
  ...
  return (
    <div className="min-h-screen bg-slate-100 dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors duration-200">
      {/* ========================================================
          PRINT CSS (Desbloqueio de rolagem)
          ======================================================== */}
            ...
            <div className="px-5 sm:px-8 py-5">
              <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-5">
                <div className="flex items-start gap-4">
                  <div className="hidden sm:flex w-12 h-12 rounded-2xl bg-slate-900 dark:bg-slate-800 items-center justify-center shrink-0">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-900">
                        ONLINE
                      </span>
                    </div>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <Button
                    ...
                    className="gap-2 bg-slate-950 hover:bg-slate-800 dark:bg-slate-100 dark:hover:bg-white dark:text-slate-950"
                  >
            ...
            <Card className="p-5 border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm transition-colors">
              <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-5">
                ...
                <Button
                  ...
                  className="gap-2 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white text-xs"
                >
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      {(professionals || [])
                        ...
                        .sort((a, b) =>
                          getProfessionalName(a).localeCompare(
                            getProfessionalName(b),
                            'pt-BR'
                          )
                        )
                        .map((professional) => (
  ...
  return (
    <button
      ...
      className={`
        ...
        ${
          active
            ? 'bg-white dark:bg-slate-800 text-slate-950 dark:text-white shadow-lg'
  ...
  return (
    <div>
      ...
      <div className="h-2 rounded-full bg-slate-100 dark:bg-slate-800 overflow-hidden">
        <div
          className="h-full rounded-full bg-slate-800 dark:bg-slate-400 transition-all"
          style={{
            width: `${numeric}%`,
          }}
        />
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-200 dark:border-slate-700 text-left">
              <th className="py-3 pr-4 text-slate-500 dark:text-slate-400">
                Profissional
              </th>

              <th className="py-3 px-4 text-slate-500 dark:text-slate-400">
                Categoria
              </th>

              <th className="py-3 px-4 text-right text-slate-500 dark:text-slate-400">
                Total
              </th>
              ...
              <th className="py-3 pl-4 text-right text-slate-500 dark:text-slate-400">
                Horas
              </th>
            </tr>
          </thead>
    ...
    <div className="space-y-5">
      {data.missingRates > 0 && (
        <div className="rounded-xl border border-amber-200 dark:border-amber-900 bg-amber-50 dark:bg-amber-950/40 p-4 flex gap-3">
          ...
          <div className="text-sm text-amber-900 dark:text-amber-200">
            ...
            registro(s) não possuem remuneração compatível.
            Esses registros foram desconsiderados do custo
            estimado.
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <KpiCard
          ...
          description="Apurado no cruzamento com a base de faturamento"
        />
  ...
  return (
    ...
    >
      <div className="report-print-shell w-full h-full max-w-[1500px] bg-white dark:bg-slate-900 rounded-2xl shadow-2xl overflow-hidden flex flex-col">
        ...
        {/* REPORT SCROLL */}
        <div className="report-print-content flex-1 overflow-y-auto bg-slate-100 dark:bg-slate-800 p-3 sm:p-8">
          <div
